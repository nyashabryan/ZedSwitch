...
control IngressProc(inout headers hdr,
    ...
    action ipv4_forward() {
        ...
        metadata.packet_length = (bit<11>)hdr.ipv4.totalLen;
        metadata.valid_bit = 1;
    }
    ...
    apply {
        ...
        if (hdr.ethernet.etherType == ETHERTYPE_TYPE_IPV4) {
            if (hdr.ipv4.isValid()) {
                ...
                if(hdr.ipv4.ttl - 1 == 0 || hdr.ipv4.srcAddr == SELFIP4ADD || hdr.ipv4.dstAddr
                == SELFIP4ADD) {
                    drop();
                } else {
